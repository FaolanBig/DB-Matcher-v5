        ...
        SheetInput2:
            ...
            for (int cnt = primaryFirstCellRow; cnt <= primaryLastCellRow; cnt++)
            {
                ...
                if (!verbose)
                {
                    ...
                    double remainingTime = timeSpanMillisecondsPerIterationHold * totalIterations;
                ...
                if (writeResults)
                {
                    using (FileStream ffstream = new FileStream(toPath, FileMode.Create, FileAccess.ReadWrite))
                    {
                        for (int i = secondaryFirstCellColumn; i <= secondaryLastCellColumn; i++)
                        {
                            ...
                            string cellValueTransferHold = fromCell.ToString();
                    ...
                    using (FileStream ffstream = new FileStream(toPath, FileMode.Create, FileAccess.Write))
                    {
                        ...
                        string cellValueTransferHold = $"LD-Value: {activeMatchPercentage}";
            ...
            Console.WriteLine();
            Console.WriteLine("Original Checksum (SHA256): " + varHold.checksumConvertedOriginal);
            Console.WriteLine("Original Checksum (SHA256): " + varHold.checksumConvertedNew);
            Console.WriteLine();
